...
public class reportMakerTest {
    ...
    public static void addDataToReport(testResultObject tRO) { // todo is this appropriate or should we reference "results.add" statically? TBD
        results.add(tRO);
    }
    public static void addDataToCSV(String output)
    { //todo check validity of "results" before running this method? Shouldn't be a case where not valid by *this point* but ¯\_(ツ)_/¯
        ...
        try {
            ...
            List<String[]> data = new ArrayList<String[]>(); // list which maintains our data to be written
            ...
            data.add(SIDINFO);
                ...
                for (int i = 0; i < noOfRow; i++) {
                    ...
                    String row = ("Test: " + results.get(i).orderOfAppearance + " ; " + results.get(i).desc + " ; Value: " + results.get(i).value + "% ; Mark received: " + results.get(i).result + " ; Contribution to total mark: " + results.get(i).result*(results.get(i).value/100));
    ...
    public void returnReport() {
       ...
       // todo convert from JUnitAssistanceVariable (list) to something comparable to our results (convert it to string or create our own list?)
